...
2. Go to the [releases](https://github.com/qMaxXen/NBTrackr/releases/latest) section of this repository and download `NBTrackr-imgpin-v2.7.0.tar.xz`.

3. Move the downloaded file to a convenient folder, then extract it using the terminal with the following command:

   ```bash
   tar -xf NBTrackr-imgpin-v2.7.0.tar.xz
   ```
4. Install the required Python packages to run NBTrackr:

   ```bash
   cd NBTrackr-imgpin-v2.7.0
   chmod +x install.sh
   ./install.sh
   ```
...
## Credits
- [Boyenn](https://twitch.tv/lazy_boyenn) for giving me the idea to make this overlay.
